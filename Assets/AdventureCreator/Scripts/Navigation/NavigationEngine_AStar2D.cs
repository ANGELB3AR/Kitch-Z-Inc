...
namespace AC
{
	...
	public class NavigationEngine_AStar2D : NavigationEngine
	{
		...
		private Dictionary<PolygonCollider2D, Grid2D> gridDict = new Dictionary<PolygonCollider2D, Grid2D> ();
		...
		public override void OnReset (NavigationMesh navMesh)
		{
			if (navMesh)
			{
				...
				{
					Grid2D grid = new Grid2D (poly, cellWidth, cellHeight);
					gridDict.Add (poly, grid);
				}
		...
		public Grid2D GetGrid (PolygonCollider2D poly)
		{
			if (gridDict.ContainsKey (poly))
			{
				return gridDict[poly];
			}
			return null;
		}
		...
		public override Vector3[] GetPointsArray (Vector3 startPosition, Vector3 targetPosition, Char _char = null)
		{
			...
			{
				if (polys[i].OverlapPoint (startPosition))
				{
					Grid2D grid = gridDict[polys[i]];
				...
				float dist = (polys[i].ClosestPoint (startPosition) - (Vector2) startPosition).sqrMagnitude;
#else
				float dist = (polys[i].bounds.center - startPosition).sqrMagnitude;
#endif
				if (dist < minDist)
				{
					minDist = dist;
			...
			if (minDistIndex >= 0)
			{
				Grid2D grid = gridDict[polys[minDistIndex]];
				AddCharHoles (grid, _char, KickStarter.sceneSettings.navMesh);
		...
		public override void DrawGizmos (GameObject navMeshOb)
		{
			if (navMeshOb)
			{
				...
				PolygonCollider2D[] polygons = navMesh.GetComponents<PolygonCollider2D> ();
				foreach (PolygonCollider2D polygon in polygons)
				{
					float cellWidth = navMesh.accuracy * 0.1f;
					float cellHeight = cellWidth;

					int gridSizeX = Mathf.RoundToInt (polygon.bounds.size.x / cellWidth);
					int gridSizeY = Mathf.RoundToInt (polygon.bounds.size.y / cellHeight);
