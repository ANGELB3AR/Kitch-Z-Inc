...
namespace AC.AStar2D
{
	...
	public class Grid2D
	{
		...
		private readonly PolygonCollider2D Polygon;
		private Node[,] nodes;
		...
		public Grid2D (PolygonCollider2D polygon, float _cellWidth, float _cellHeight)
		{
			Polygon = polygon;
		...
		public void Rebuild ()
		{
			gridSizeX = Mathf.RoundToInt (Polygon.bounds.size.x / cellWidth);
			gridSizeY = Mathf.RoundToInt (Polygon.bounds.size.y / cellHeight);

			nodes = new Node[gridSizeX, gridSizeY];

			Vector2 bottomLeft = (Vector2) Polygon.bounds.center - (Vector2.right * Polygon.bounds.size.x / 2f) - (Vector2.up * Polygon.bounds.size.y / 2f);
		...
		{
			Vector2 bottomLeft = (Vector2) Polygon.bounds.center - (Vector2.right * Polygon.bounds.size.x / 2f) - (Vector2.up * Polygon.bounds.size.y / 2f);
			float percentX = (position.x - bottomLeft.x) / Polygon.bounds.size.x;
			float percentY = (position.y - bottomLeft.y) / Polygon.bounds.size.y;
		...
		public bool IsLineInside (Vector2 startPos, Vector2 endPos)
		{
			...
			Vector2 correctedStart = InverseTransformPoint (startPos);
			Vector2 correctedEnd = InverseTransformPoint (endPos);
			...
			for (int p = 0; p < Polygon.pathCount; p++)
			{
				...
				for (int i = 0; i < path.Length; i++)
				{
					...
					float intersectP = IntersectLineSegments2D (correctedStart, correctedEnd, path[i], path[j]);
					if (intersectP >= 0f)
					{
						if (Approximately (intersectP, 0f))
						{
							// Start on line
						}
						else if (Approximately (intersectP, 1f))
						{
							// Ends on line
						}
						else
						{
							if (IntersectLineSegment2D (correctedStart, path[i], path[j]) && IntersectLineSegment2D (correctedEnd, path[i], path[j]))
							{
								// Points both on same line
								continue;
							}
							return false;
						}
			...
			if (!IsPointInside ((startPos + endPos) / 2f))
			{
				return false;
			}
			return true;
		...
		{
			return Polygon.transform.InverseTransformPoint (position);
		}
